...
public class ProductFiltrController {
    ...
    void initialize() {
        ...
        denProduct_btn.setOnAction(event -> {
            filtrByDens(productParam_field.getText());
        });

        typeProduct_btn.setOnAction(event -> {
            filtrByType(productParam_field.getText());
        });

        numbProduct_btn.setOnAction(event -> {
            filtrByNumb(productParam_field.getText());
        });
    ...
    public void filtrByDens(String prDen){
        DataSending dataSending = new DataSending();
        int result = dataSending.filtrProductDes(prDen);
        if(result > 0){
            Alert alert = new Alert(Alert.AlertType.INFORMATION);

            alert.setTitle("Information");
            alert.setHeaderText(null);
            alert.setContentText("Фильтрация выполнена успешно!!");
            ...
            alert.setTitle("Information");
            alert.setHeaderText(null);
            alert.setContentText("Фильтрация не выполнена!");

            alert.showAndWait();
        }
    }
    public void filtrByType(String prType){
        DataSending dataSending = new DataSending();
        int result = dataSending.filtrProductType(prType);
        if(result > 0){
            Alert alert = new Alert(Alert.AlertType.INFORMATION);
    ...
    public void filtrByNumb(String prNumb){
        DataSending dataSending = new DataSending();
        int result = dataSending.filtrProductNumb(prNumb);
        if(result > 0){
            Alert alert = new Alert(Alert.AlertType.INFORMATION);
